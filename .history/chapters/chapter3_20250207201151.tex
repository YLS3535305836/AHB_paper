...
实验结果表明（见表\ref{tab:MR-PPFN ablation res}），不同融合策略对模型性能具有显著影响。在PointNet-BiLSTM架构中，乘性融合与注意力融合的组合取得最优F1分数0.9444，较单一融合策略提升约1.5-2.3个百分点。值得注意的是，注意力机制在多数模型中展现出关键作用，如PointCNN-BiLSTM单独使用注意力融合时F1分数达0.9608，表明其能有效捕捉跨模态特征关联。

不同点云处理网络对融合策略呈现差异化响应：基于图卷积的DGCNN-BiLSTM在仅使用乘性融合时取得最佳性能（F1=0.9062），这与其局部几何特征提取特性相关；而基于规则卷积的SpiderCNN-BiLSTM在注意力+乘性融合时达到最高准确率98.46$\%$，说明其全局特征需要更复杂的交互机制。特别地，RSNet-BiLSTM在三者融合时性能最优（F1=0.7534），验证了多策略融合对复杂网络架构的适应性。

图\ref{fig:Adaptive_fusion_ablation_results}显示，所有模型在完整融合策略下的平均准确率达95.17$\%$，较最优单一策略提升2.34个百分点。这证明本文提出的自适应加权机制能有效整合不同融合方式的优势，其中注意力权重占比平均达43.6$\%$，表明时序-空间特征交互在姿态识别中的核心作用。
...
\begin{table}[htbp]
    ...
    \begin{subtable}{\linewidth}
        ...
        \begin{tabular}{ccc|cccc}
            ...
            \midrule
            \ding{51} & & & 98.77 & 0.9440 & 0.9006 & 0.9196 \\
            & \ding{51} & & 98.48 & 0.9473 & 0.9227 & 0.9312 \\
            & & \ding{51} & 98.65 & 0.9393 & 0.9399 & 0.9361 \\
            \ding{51} & \ding{51} & & 98.25 & 0.9498 & 0.9215 & 0.9319 \\
            \ding{51} & & \ding{51} & 98.11 & 0.9549 & 0.9227 & 0.9346 \\
            & \ding{51} & \ding{51} & \textbf{98.37} & \textbf{0.9672} & \textbf{0.9252} & \textbf{0.9444} \\
            \ding{51} & \ding{51} & \ding{51} & 98.27 & 0.9402 & 0.9298 & 0.9322 \\
            \bottomrule
        \end{tabular}
...
\begin{table}[htbp]
    ...
    \begin{subtable}{\linewidth}
        ...
        \begin{tabular}{ccc|cccc}
            ...
            拼接融合 & 乘性融合 & 注意力融合 & 准确率 (\%) & 精确率 & 召回率 & $F_1$分数 \\
            \midrule
            \ding{51} & & & 98.91 & 0.9384 & 0.9577 & 0.9468 \\
            & \ding{51} & & 99.08 & 0.9598 & 0.9523 & 0.9540 \\
            & & \ding{51} & 99.01 & 0.9656 & 0.9652 & 0.9608 \\
            \ding{51} & \ding{51} & & 98.96 & 0.9323 & 0.9352 & 0.9295 \\
            \ding{51} & & \ding{51} & 98.11 & 0.9549 & 0.9227 & 0.9346 \\
            & \ding{51} & \ding{51} & \textbf{99.24} & \textbf{0.9558} & \textbf{0.9461} & \textbf{0.9498} \\
            \ding{51} & \ding{51} & \ding{51} & 99.15 & 0.9487 & 0.9475 & 0.9475 \\
            \bottomrule
        \end{tabular}
    ...
    \begin{subtable}{\linewidth} 
        ...
        \begin{tabular}{ccc|cccc}
            ...
            拼接融合 & 乘性融合 & 注意力融合 & 准确率 (\%) & 精确率 & 召回率 & $F_1$分数 \\
            \midrule
            \ding{51} & & & 98.65 & 0.9641 & 0.9603 & 0.9589 \\
            & \ding{51} & & 97.94 & 0.9671 & 0.9712 & 0.9686 \\
            & & \ding{51} & 98.46 & 0.9738 & 0.9656 & 0.9694 \\
            \ding{51} & \ding{51} & & 98.30 & 0.9570 & 0.9452 & 0.9469 \\
            \ding{51} & & \ding{51} & 98.41 & 0.9824 & 0.9704 & 0.9754 \\
            & \ding{51} & \ding{51} & \textbf{97.96} & \textbf{0.9569} & \textbf{0.9548} & \textbf{0.9551} \\
            \ding{51} & \ding{51} & \ding{51} & 98.30 & 0.9475 & 0.9514 & 0.9490 \\
            \bottomrule
        \end{tabular}
    ...
    \begin{subtable}{\linewidth}
        ...
        \begin{tabular}{ccc|cccc}
            ...
            拼接融合 & 乘性融合 & 注意力融合 & 准确率 (\%) & 精确率 & 召回率 & $F_1$分数 \\
            \midrule
            \ding{51} & & & 82.65 & 0.7167 & 0.7041 & 0.7015 \\
            & \ding{51} & & 87.12 & 0.7085 & 0.6895 & 0.6871 \\
            & & \ding{51} & 81.37 & 0.7555 & 0.6780 & 0.6839 \\
            \ding{51} & \ding{51} & & 86.67 & 0.7468 & 0.6782 & 0.6798 \\
            \ding{51} & & \ding{51} & 82.77 & 0.7467 & 0.7268 & 0.7261 \\
            & \ding{51} & \ding{51} & 87.00 & 0.6949 & 0.7057 & 0.6932 \\
            \ding{51} & \ding{51} & \ding{51} & \textbf{88.33} & \textbf{0.7796} & \textbf{0.7708} & \textbf{0.7534} \\
            \bottomrule
        \end{tabular}
